    ...
    <div class="flex">
        <div class="categorie max-w-lg w-64 p-10 md:block hidden bg-slate-200">
            <ul class="flex flex-col gap-3">
                ...
                <li class="text-2xl p-2 bg-black text-white cursor-pointer" @click="toKinderShop">
                    <a href="#">Kinder</a>
                </li>
        ...
        <main class="itemList flex flex-col w-full md:p-10 py-10 md:pt-10 bg-slate-100">
            <div
                class="flex p-5 justify-center items-center w-full gap-4 bg-gradient-to-r from-yellow-200 via-yellow-300 to-yellow-400">
                <div class="text-black text-2xl">
                    <h3 class="md:text-4xl text-2xl font-extrabold">Los geht's zur Schule!</h3>
                    <p class="md:block hidden">Ergonomisch und extra leichte Taschen für eine gesunde Zukunft</p>
                </div>
                <div class="flex justify-center gap-4 md:px-2">
                    <img src="../img/KinderSchoolbagback.jpeg" class="md:w-56 h-72 hidden md:block " alt="">
                    <img src="../img/Bild-from-laden.jpeg" class="md:w-56" alt="">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 md:gap-4 gap-2 md:py-10">
                <div v-for="item in itemListK" :key="item.id" class=" flex bg-slate-200 rounded-2xl p-2">
                    <img class="rounded-2xl itemimagesk" :src="item.url" alt="">
...
export default {
    ...
    mounted() {
        ...
        this.$appAxios.get("/itemListK").then(itemListK_response => {
            this.itemListK = itemListK_response?.data.filter(item => item.itemID === 1) || [];
        })
    },
}
</script>
